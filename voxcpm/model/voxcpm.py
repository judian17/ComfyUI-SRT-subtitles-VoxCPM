...
class VoxCPMModel(nn.Module):
    ...
    def _generate(
        ...
        if len(prompt_wav_path) == 0:
            ...
            try:
                audio_data, sr = sf.read(prompt_wav_path, dtype='float32')
            except Exception as e:
                print(f"[VoxCPM-Local-Fix] soundfile.read failed: {e}")
                raise e

            # soundfile 返回 [samples, channels] 或 [samples]
            # torchaudio.load 返回 [channels, samples]
            if audio_data.ndim == 2:
                audio_data = audio_data.T  # [samples, channels] -> [channels, samples]
            else:
                audio_data = audio_data[None, :]  # [samples] -> [1, samples]
